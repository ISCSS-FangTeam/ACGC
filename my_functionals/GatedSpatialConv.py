...
class GatedSpatialConv2d(_ConvNd):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1,
        ...
        kernel_size = _pair(kernel_size)  # _pair(3)会生成(3x3)
        stride = _pair(stride)
        ...
        self._gate_conv = nn.Sequential(
            mynn.Norm2d(in_channels+1),  # 正则化 通道数是in_channels+1
            nn.Conv2d(in_channels+1, in_channels+1, 1),  # 再接卷积
            nn.ReLU(), 
    ...
    def forward(self, input_features, gating_features):
        ...
        input_features = (input_features * (alphas + 1))   # alphas
